...
def get_waqi_data(lat, lng):
    ...
    try:
        ...
        if res.get("status") == "ok":
            data    = res["data"]
            station_name = data.get("city", {}).get("name", "CPCB Station")
            
            if "shanghai" in station_name.lower() or "china" in station_name.lower():
                return None
...
def get_osm_routes(start_lat, start_lng, end_lat, end_lng):
    ...
    try:
        ...
        for idx, feature in enumerate(features):
            ...
            # ── DYNAMIC TRAFFIC REALITY CHECK (State-of-the-Art Congestion Modeling) ──
            raw_duration_min = summary.get("duration", 0) / 60
            
            # Get current hour in Bangalore (IST is UTC+5:30)
            now_utc = datetime.utcnow()
            blr_hour = (now_utc.hour + 5) % 24 # Simple IST approximation
            if now_utc.minute >= 30: blr_hour = (blr_hour + 1) % 24

            # Define Congestion Factors based on Bangalore Reality
            # Peak (8-11 AM, 5-9 PM): 3.5x - 4.0x
            # Mid-day (12-4 PM): 2.2x
            # Night (11 PM - 6 AM): 1.2x
            
            if 8 <= blr_hour <= 11:
                traffic_multiplier = 3.6  # Morning Rush
            elif 17 <= blr_hour <= 21:
                traffic_multiplier = 4.2  # Evening Gridlock
            elif 22 <= blr_hour or blr_hour <= 6:
                traffic_multiplier = 1.3  # Night Clear
            else:
                traffic_multiplier = 2.4  # Standard Bangalore Traffic

            # Apply only if in Bangalore region
            is_blr = (12.7 <= start_lat <= 13.2)
            if not is_blr: traffic_multiplier = 1.2 # Generic traffic for other areas
...
def recommend_route(start_lat, start_lng, end_lat, end_lng, condition="normal", start_name="", end_name=""):
    ...
    for route in routes:
        ...
        # --- ADMIN OVERRIDE: Unsafe Road Check ---
        unsafe_penalty = 0
        unsafe_reasons = []
        try:
            from models import UnsafeRoad
            unsafe_list = UnsafeRoad.query.all()
            for ur in unsafe_list:
                for coord in route["coords"]: # coord is [lng, lat]
                    dist = math.sqrt((coord[1] - ur.lat)**2 + (coord[0] - ur.lng)**2) * 111
                    if dist < ur.radius:
                        unsafe_penalty += 500 # Huge penalty
                        if ur.reason not in unsafe_reasons:
                            unsafe_reasons.append(ur.reason)
                        break
        except Exception as e:
            print(f"[ADMIN] Error checking unsafe roads: {e}")

        # Normalize for combined score (Scale: WES ~150 is high, Distance ~15km is typical)
        norm_wes = (avg_wes + unsafe_penalty) / 150
        ...
        results.append({
            ...
            "avg_wes": avg_wes,
            "risk": "DANGER (Admin Restriction)" if unsafe_penalty > 0 else get_risk(avg_wes),
            "unsafe_warning": " ".join(unsafe_reasons) if unsafe_reasons else None,
